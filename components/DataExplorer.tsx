    ...
    <div>
      ...
      <div className="flex flex-col gap-2 sm:flex-row">
        <textarea
          ...
          rows={2}
          spellCheck={false}
          className="w-full flex-1 rounded-md border border-gray-300 px-3 py-2 font-mono text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-brand"
        />
        <button
          type="button"
          onClick={run}
          disabled={loading}
          className="h-fit rounded-md bg-brand px-4 py-2 text-sm font-medium text-white hover:bg-brand-dark disabled:opacity-50"
        >
          {loading ? 'Running…' : 'Run'}
        </button>
      </div>
